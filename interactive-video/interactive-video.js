...
var $interactivevideo = {
    ...
    init: function () {
        ...
        this.idevicePath = this.isInExe
            ...
            : $('.idevice_node.interactive-video').eq(0).attr('data-idevice-path');
    ...
    loadSCORM_API_wrapper: function () {
        if (typeof (pipwerks) == 'undefined') $exe.loadScript(this.scormAPIwrapper, '$interactivevideo.loadSCOFunctions()');
    ...
    enable: function () {
        ...
        if (showResults) {
            html += '\
				 <div class="js-required">\
					 <h2 id="activity-results-toggler"><a href="#activity-results" onclick="$interactivevideo.resultsViewer.toggle(this);return false" class="show">'+ i18n.results + '</a></h2>\
        ...
        var playContent = '<a href="#" onclick="$interactivevideo.cover.hide(true);return false" id="start-link">' + i18n.start + '</a>';
        ...
        if (this.type == 'mediateca') {
            eXe.app.loadScript("https://mediateca.educa.madrid.org/includes/player/exelearning/jwplayer.js", "$interactivevideo.ready()");
            return;
            ...
            $("#player").html('<video width="448" height="356" class="mejs__player" controls="controls"><source src="' + this.file + '" /></video>');
        }
    ...
    saveEvaluation: function () {
        ...
        options.scorerp = ($interactivevideo.score * 10) / $interactivevideo.numSlides;
        $exeDevices.iDevice.gamification.report.saveEvaluation(options, options.isInExe)
    },
    ...
    updateScore: function (question, result) {
        ...
        if (InteractiveVideo.scorm.isScorm == 1) {
            var scoref = (($interactivevideo.score * 10) / $interactivevideo.numSlides).toFixed(2);
            $('#interactiveRepeatActivity').text(InteractiveVideo.i18n.msgYouScore + ': ' + scoref)
    ...
    cover: {
        hide: function (play) {
            ...
            if (InteractiveVideo && InteractiveVideo.slides != undefined && InteractiveVideo.slides.length > 0) {
                if (InteractiveVideo.ideviceID && InteractiveVideo.evaluation && InteractiveVideo.evaluationID.length > 0) {
                    let goptions = $interactivevideo.getOptions(InteractiveVideo);
                    $exeDevices.iDevice.gamification.report.updateEvaluationIcon(
                        goptions,
                        $interactivevideo.isInExe,
                    );
                }
                if (InteractiveVideo && InteractiveVideo.scorm && InteractiveVideo.scorm.isScorm > 0) {
                    let goptions = $interactivevideo.getOptions(InteractiveVideo);
                    $exeDevices.iDevice.gamification.scorm.registerActivity(goptions);
                }
                ...
                $interactivevideo.numSlides = InteractiveVideo.scoreNIA ? $interactivevideo.scoreSlides.length : activeSlide;
    ...
    controls: {
        play: function () {
            if ($interactivevideo.type == "mediateca") {
                ...
                if ($interactivevideo.mediaElementReady == false) {
                    ...
                    if ($interactivevideo.mediaElementVideo.mediaelementplayer) {
                    ...
                    setTimeout(function () {
                        if ($interactivevideo.extension == 'flv') $(".mejs-overlay-button").trigger("click");
        ...
        stop: function () {
            ...
            else if ($interactivevideo.type == "youtube") $interactivevideo.player.pauseVideo();
            else if ($interactivevideo.type == "local") {
                if ($interactivevideo.extension == 'flv') $(".mejs-pause button").trigger("click");
    ...
    getTypeAndId: function () {
        ...
        if (as.length == 1) {
            ...
            alert(InteractiveVideo.i18n.error + " - " + InteractiveVideo.i18n.dataError);
        }
    ...
    checkSlides: function () {
        if ($interactivevideo.isSeek) {
            if ($("BODY").hasClass("active")) {
                ...
                if (c == "image" || c == "text") {
                    if (!InteractiveVideo.slides[$interactivevideo.visibleSlide].endTime) {
        ...
        if (this.type == "mediateca") {
            ...
            $interactivevideo.player = new YT.Player('player', {
                ...
                events: {
                    ...
                    'onStateChange': function (e) {
                        $interactivevideo.hasPlayed = true;
                        $interactivevideo.youtubeCounter = setInterval(function () {
                            if ($interactivevideo.player && typeof $interactivevideo.player.getCurrentTime === "function") {
                                $interactivevideo.track($interactivevideo.player.getCurrentTime());
                            } else {
            ...
            if ($interactivevideo.extension == "flv") {
                ...
                $interactivevideo.localCounter = setInterval(function () {
                    if (typeof $interactivevideo.hourToSeconds($("#player .mejs-currenttime")) !== 'undefined') {
                        $interactivevideo.track($interactivevideo.hourToSeconds($("#player .mejs-currenttime").eq(0).text()));
                    } else {
                ...
                $interactivevideo.mediaElementVideo[0].addEventListener('playing', function (e) {
                    $interactivevideo.localCounter = setInterval(function () {
                        if (typeof $interactivevideo.mediaElementVideo !== 'undefined') {
                            $interactivevideo.track($interactivevideo.mediaElementVideo[0].currentTime);
                        } else {
                            clearInterval($interactivevideo.localCounter);
                        }
                    }, 500);
    ...
    complete: function () {
        ...
        $(document).ready(function () {
            if ($interactivevideo.type === 'local') {
                const ivStyle = $interactivevideo.mediaElementVideo[0]?.getAttribute("style") || '';
                if (!ivStyle.includes("width: 448px; height: 356px;")) {
                    $interactivevideo.mediaElementVideo[0].setAttribute("style", `${ivStyle} width: 448px; height: 356px;`.trim());
                }
            }
    ...
    orderSlides: function () {
        ...
        for (var slide in InteractiveVideo.slides) {
            sortable.push([$interactivevideo.secondsToHour(InteractiveVideo.slides[slide].startTime), InteractiveVideo.slides[slide]])
        }
        sortable.sort();
        var sorted = [];
    ...
    resultsViewer: {
        create: function () {
            ...
            var cover = '\
					 <tr class="odd">\
						 <td>&nbsp;</td>\
						 <td><a href="#" onclick="$interactivevideo.cover.show();return false">'+ i18n.cover + '</a> </td>\
						 <td>&nbsp;</td>\
					 </tr>';
            // }
            var questions = InteractiveVideo.slides;
            for (var i = 0; i < questions.length; i++) {
                var e = questions[i];
                var title = title = "Actividad - " + $interactivevideo.typeNames[e.type];
                if (e.type == "image" || e.type == "text") title = "Información - " + $interactivevideo.typeNames[e.type];
                var result = "-";
                var k = "odd";
                if (i % 2 == 0) k = "even";
                if ($interactivevideo.type == "local" && $interactivevideo.extension == "flv") {
                    html += '\
						 <tr class="'+ k + '">\
							 <td class="order"><span>'+ (i + 1) + '</span> </td>\
							 <td><span>'+ $interactivevideo.secondsToHour(e.startTime) + '</span> </td>\
                    ...
                    html += '\
						 <tr class="'+ k + '">\
							 <td class="order"><span>'+ (i + 1) + '</span> </td>\
							 <td><a href="#" onclick="$interactivevideo.seek('+ i + ');return false">' + $interactivevideo.secondsToHour(e.startTime) + '</a> </td>\
							 <td class="result"><span>'+ result + '</span> </td>\
						 </tr>';
                }
            }
        ...
        getFinalResult: function (tds) {
            ...
            tds.each(
                function () {
                    ...
                    if (t.indexOf("%") != -1) {
                        result += parseFloat(t);
                        counter++;
                    } else if (t == InteractiveVideo.i18n.seen && InteractiveVideo.scoreNIA) {
                        result += 100;
                        counter++;
                    }
                }
            ...
            if (!InteractiveVideo.scoreNIA && counter >= $interactivevideo.numSlides) complete = true;
            if (complete) {
                $("#resultsSummary").html("<span>" + $interactivevideo.formatResult((result / counter).toFixed(2)) + "%</span>");
    ...
    seek: function (order) {
        if ($interactivevideo.visibleSlide == order && $("BODY").hasClass("active")) {
            return false;
        }
        $interactivevideo.isSeek = true;
        if ($interactivevideo.hasPlayed != true) {
            $interactivevideo.controls.seek(InteractiveVideo.slides[order].startTime - 1);
        } else {
            $interactivevideo.controls.seek(InteractiveVideo.slides[order].startTime);
        }
    },
    ...
    slide: {
        enable: function (slide, e, order) {
            ...
            if (e.type == "text") {
                ...
                img.onload = function () {
                    slide.html($interactivevideo.getImage(e, img.width, img.height, img.src));
                    for (var i = 0; i < InteractiveVideo.slides.length; i++) {
                        if (e == InteractiveVideo.slides[i]) {
                            $interactivevideo.updateResult(i, i18n.seen);
                            e.results = {
            ...
            $("#slide").before('<a href="#slide" id="slide-link" class="sr-av">' + InteractiveVideo.i18n.slide + '</a>');
            $("#slide-link").focus();
        },
        show: function (e, order) { // e = the object
            ...
            if ($interactivevideo.isFullScreen()) {
                ...
                if ($interactivevideo.type != "local") alert(InteractiveVideo.i18n.fsWarning);
            }
    ...
    dropdownActivity: {
        create: function (slide, id, order, e) {
            var html = $interactivevideo.getForm.header(id, order, 'dropdownActivity');
            html += '<p class="question">' + e.text + '</p>';
            html += '<p class="actions">';
            html += $interactivevideo.getForm.sendButton(id, order, 'dropdownActivity');
            html += '</p>';
            ...
            if (currentQuestion.results) {
                ...
                if (currentQuestion.results.score == 100) {
                    $interactivevideo.msg("success", i18n.right, id + "dropdownActivityForm");
                } else {
                    $interactivevideo.msg("error", i18n.wrong, id + "dropdownActivityForm", $interactivevideo.dropdownActivity.getRightAnswer(rightAnswers));
                }
        ...
        validate: function (question, id) {
            ...
            if (!answered) {
                $interactivevideo.msg("info", "Aún no has completado la actividad", e.id);
                return false;
            }
            ...
            if (errors) {
                ...
                var extra = $interactivevideo.dropdownActivity.getRightAnswer(rightAnswers);
                $interactivevideo.msg("error", i18n.wrong, id, extra);
                $interactivevideo.dropdownActivity.saveResults(question, result, slide, selectedValues);
            } else {
                $interactivevideo.msg("success", i18n.right, id);
                $interactivevideo.dropdownActivity.saveResults(question, 100, slide, selectedValues);
            }
    ...
    sortableList: {
        create: function (slide, id, order, e) {
            var i18n = InteractiveVideo.i18n;
            var currentQuestion = InteractiveVideo.slides[order];
            var html = $interactivevideo.getForm.header(id, order, 'sortableList');
            ...
            html += '<p class="actions">';
            html += $interactivevideo.getForm.sendButton(id, order, 'sortableList');
            html += '</p>';
            ...
            if (currentQuestion.results) {
                ...
                if (currentQuestion.results.score == 100) {
                    $interactivevideo.msg("success", i18n.right, id + "sortableListForm");
                } else {
                    $interactivevideo.msg("error", i18n.wrong, id + "sortableListForm", $interactivevideo.sortableList.getRightAnswer(e.items));
                }
            } else {
        ...
        getLinksHTML: function (i, id) {
            var i18n = InteractiveVideo.i18n;
            return '<span> <a href="#" class="up" onclick="$interactivevideo.sortableList.sortList(this,' + i + ',' + (i - 1) + ',\'' + id + '\');return false" title="' + i18n.up + ': ' + (i + 1) + ' → ' + i + '"><span class="sr-av">' + i18n.up + '</span></a> <a href="#" class="down" onclick="$interactivevideo.sortableList.sortList(this,' + i + ',' + (i + 1) + ',\'' + id + '\');return false" title="' + i18n.down + ': ' + (i + 1) + ' → ' + (i + 2) + '"><span class="sr-av">' + i18n.down + '</span></a></span>';
        },
        updateLinks: function (id) {
            ...
            lis.each(function (i) {
                ...
                this.innerHTML += $interactivevideo.sortableList.getLinksHTML(i, id);
            });
        },
        getListHTML: function (lis, id) {
            ...
            for (var i = 0; i < lis.length; i++) {
                ...
                html += '>' + lis[i] + $interactivevideo.sortableList.getLinksHTML(i, id) + '</li>';
            }
            return html;
        },
        sortList: function (e, a, b, id) { // LI - FROM - TO
            ...
            list.html($interactivevideo.sortableList.getListHTML(newList, id)).sortable();
        },
        saveResults: function (question, result, slide, selectedValues) {
            var currentQuestion = InteractiveVideo.slides[question];
            if (!currentQuestion.results) {
        ...
        getRightAnswer: function (rightAnswers) {
            ...
            if (rightAnswer != "") rightAnswer = "<ul>" + rightAnswer + "</ul>";
            return "<p><strong>" + InteractiveVideo.i18n.rightAnswer + " </strong>" + rightAnswer + "</p>";
        },
        validate: function (question, id) {
            var i18n = InteractiveVideo.i18n;
            var slide = $("#" + id);
            ...
            if (error) {
                ...
                $interactivevideo.msg("error", i18n.wrong, id, extra);
                $interactivevideo.sortableList.saveResults(question, 0, slide, selectedValues);
            } else {
                $interactivevideo.msg("success", i18n.right, id);
                $interactivevideo.sortableList.saveResults(question, 100, slide, selectedValues);
            }
        }
    },
    clozeActivity: {
        create: function (slide, id, order, e) {
            var i18n = InteractiveVideo.i18n;
            var html = $interactivevideo.getForm.header(id, order, 'clozeActivity');
            html += '<p class="question">' + e.text + '</p>';
            html += '<p class="actions">';
            html += $interactivevideo.getForm.sendButton(id, order, 'clozeActivity');
            html += '</p>';
            ...
            if (currentQuestion.results) {
                ...
                if (currentQuestion.results.score == 100) {
                    $interactivevideo.msg("success", i18n.right, id + "clozeActivityForm");
                } else {
                    $interactivevideo.msg("error", i18n.wrong, id + "clozeActivityForm", $interactivevideo.clozeActivity.getRightAnswer(rightAnswers));
                }
                var selectedValues = currentQuestion.results.selectedValues;
        ...
        validate: function (question, id) {
            ...
            if (errors) {
                var result = (rightAnswered * 100 / rightAnswers.length).toFixed(2);
                var extra = $interactivevideo.clozeActivity.getRightAnswer(rightAnswers);
                $interactivevideo.msg("error", i18n.wrong, id, extra);
                $interactivevideo.clozeActivity.saveResults(question, result, slide, selectedValues);
            } else {
                $interactivevideo.msg("success", i18n.right, id);
                $interactivevideo.clozeActivity.saveResults(question, 100, slide, selectedValues);
            }
    ...
    matchElements: {
        create: function (slide, id, order, e) {
            var i18n = InteractiveVideo.i18n;
            var html = $interactivevideo.getForm.header(id, order, 'matchElements');
            if (e.text && e.text != "") html += '<div>' + e.text + '</div>';
            ...
            html += '</div>';
            html += '<p class="actions">';
            html += $interactivevideo.getForm.sendButton(id, order, 'matchElements');
            html += '</p>';
            ...
            if (currentQuestion.results) {
                activityForm.addClass("disabled");
                if (currentQuestion.results.score == 100) {
                    $interactivevideo.msg("success", i18n.right, id + "matchElementsForm");
                } else {
                    $interactivevideo.msg("error", i18n.wrong, id + "matchElementsForm", $interactivevideo.matchElements.getRightAnswer(pairs));
                }
                var selectedValues = currentQuestion.results.selectedValues;
        ...
        validate: function (question, id) {
            ...
            if (rightAnswered == pairs.length) {
                // 100% right
                $interactivevideo.msg("success", i18n.right, id);
                $interactivevideo.matchElements.saveResults(question, 100, slide, selectedValues);
            } else {
                // < 100%;
                var extra = $interactivevideo.matchElements.getRightAnswer(pairs);
                $interactivevideo.msg("error", i18n.wrong, id, extra);
                // %
                var result = (rightAnswered * 100 / pairs.length).toFixed(2);
                $interactivevideo.matchElements.saveResults(question, result, slide, selectedValues);
            }
    ...
    getForm: {
        header: function (id, order, name) {
            if ($interactivevideo.isInExe) return '<div id="' + id + name + 'Form">';
            return '<form id="' + id + name + 'Form" action="#" onsubmit="$interactivevideo[\'' + name + '\'].validate(' + order + ',this.id);return false">';
        },
        footer: function () {
        ...
        sendButton: function (id, order, name) {
            var i18n = InteractiveVideo.i18n;
            if ($interactivevideo.isInExe) return '<input type="button" name="' + id + name + 'FormSubmit" id="' + id + name + 'FormSubmit" value="' + i18n.check + '" onclick="$interactivevideo[\'' + name + '\'].validate(' + order + ',\'' + id + name + 'Form\');return false" />'
    ...
    singleChoice: {
        create: function (slide, id, order, e) {
            var i18n = InteractiveVideo.i18n;
            var html = $interactivevideo.getForm.header(id, order, 'singleChoice');
            ...
            html += '</div>';
            html += '<p class="actions">';
            html += $interactivevideo.getForm.sendButton(id, order, 'singleChoice');
            html += '</p>';
            ...
            if (currentQuestion.results) {
                ...
                if (currentQuestion.results.score == 100) {
                    $interactivevideo.msg("success", i18n.right, id + "singleChoiceForm");
                } else {
                    $interactivevideo.msg("error", i18n.wrong, id + "singleChoiceForm", $interactivevideo.singleChoice.getRightAnswer(currentQuestion.answers));
                }
            }
        ...
        validate: function (question, id) {
            ...
            if (selected.length == 0) {
                ...
                if (questions[selectedValue][1] == 1) {
                    $interactivevideo.msg("success", i18n.right, id);
                    $interactivevideo.singleChoice.saveResults(question, 100, slide, selectedValue);
                } else {
                    var extra = $interactivevideo.singleChoice.getRightAnswer(questions);
                    $interactivevideo.msg("error", i18n.wrong, id, extra);
                    $interactivevideo.singleChoice.saveResults(question, 0, slide, selectedValue);
                }
            }
        }
    },
    multipleChoice: {
        create: function (slide, id, order, e) {
            var i18n = InteractiveVideo.i18n;
            var html = $interactivevideo.getForm.header(id, order, 'multipleChoice');
            html += '<p class="question">' + e.question + '</p>';
            html += '<div class="answers">';
            for (var i = 0; i < e.answers.length; i++) {
                html += '<p>';
            ...
            html += '</div>';
            html += '<p class="actions">';
            html += $interactivevideo.getForm.sendButton(id, order, 'multipleChoice');
            html += '</p>';
            ...
            if (currentQuestion.results) {
                ...
                if (currentQuestion.results.score == 100) {
                    $interactivevideo.msg("success", i18n.right, id + "multipleChoiceForm");
                } else {
                    $interactivevideo.msg("error", i18n.wrong, id + "multipleChoiceForm", $interactivevideo.multipleChoice.getRightAnswer(currentQuestion.answers));
                }
            }
        },
        saveResults: function (question, result, slide, selectedValues) {
        ...
        getRightAnswer: function (questions) {
            ...
            rightAnswer += "</ul>";
            return "<p><strong>" + InteractiveVideo.i18n.rightAnswer + " </strong></p>" + rightAnswer;
        },
        validate: function (question, id) {
            var i18n = InteractiveVideo.i18n;
            var slide = $("#" + id);
            ...
            if (selected.length == 0) {
                ...
                if (error) {
                    var extra = $interactivevideo.multipleChoice.getRightAnswer(questions);
                    $interactivevideo.msg("error", i18n.wrong, id, extra);
                    // %
                    var result = (rightAnswered * 100 / rightAnswers).toFixed(2);
                    $interactivevideo.multipleChoice.saveResults(question, result, slide, selectedValues);
                } else {
                    $interactivevideo.msg("success", i18n.right, id);
                    // To do: save more than one selected option
                    $interactivevideo.multipleChoice.saveResults(question, 100, slide, selectedValues);
                }
            }
        }
    },
    track: function (position) {
        if (typeof InteractiveVideo === "undefined" || typeof InteractiveVideo.slides == "undefined") return;
    ...
    getImage: function (e, w, h, src) {
        ...
        return '<img src="' + src + '" alt="' + e.description + '" class="' + css + '" width="' + newW + '" height="' + newH + '" style="display:block;margin-top:' + ((maxH - newH) / 2) + 'px" /><a href="' + src + '" target="_blank">' + InteractiveVideo.i18n.newWindow + '</a>';
    },

    observeMutations: function (element) {
        if (!element) return;

        if (!$interactivevideo.observers) $interactivevideo.observers = new Map();

        if ($interactivevideo.observers.has(element)) return $interactivevideo.observers.get(element);
